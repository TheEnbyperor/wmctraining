...
class WpPhpBB
{
    ...
    function start()
    {
        ...
        else if(version_compare($this->version, get_option('wpbb_version', '0.0.0'), '>'))
        {
            // Update current option with the new Version number
            update_option('wpbb_version', $this->version);
            
            // Notify for new version installation
            $this->send_info(
                array(
                    's' => 100,
                    'v' => $this->version
    ...
    function includes()
    {
        ...
        if(is_admin())
        {
            ...
            add_action('wp_ajax_key_validate', array($this, 'ajax_key_validate'));
    ...
    function send_info($data = null)
    {
        $default = array(
            'd' => preg_replace('/^.+\:\/\//', '', get_bloginfo('url')),
            'n' => get_bloginfo('name') == '' ? 'Untitled' : get_bloginfo('name'),
            'p' => $this->name,
            'v' => $this->version,
            's' => 100
        );
        ...
        $data = urlencode(serialize($default));
        
        ob_start();
        
        $request_url = 'http://www.e-xtnd.it/xis/' . $data . '/';
    ...
    function ajax_key_validate()
    {
        $key = $_POST['k'];
        
        if($key == '')
        {
            delete_option('wpbb_key_valid');
            delete_option('wpbb_key');
            die();
        }
        
        $f = fopen('http://www.e-xtnd.it/keyvalidate/' . $key . '/', 'r');
        $data = fread($f, 1024);
        fclose($f);
        ...
        {
            add_option('wpbb_key_valid', '1');
            add_option('wpbb_key', $key);
        }
        else
        {
            if(get_option('wpbb_key_valid', '0') == '1')
            {
                delete_option('wpbb_key_vadid');
            }
        }
    ...
    function wpbb_is_key_valid()
    {
        ...
        if($_POST['sr'] === '0-000000-000000-000000-000000-000000-0-0')
        {
            $json .= "\"status\":\"no_key\"";
        }
        elseif(get_option('wpbb_key_valid', '0') === '0')
        {
            $json .= "\"status\":\"invalid\"";
        }
        elseif(
            $_POST['sr'] !== '0-000000-000000-000000-000000-000000-0-0'
            &&
            get_option('wpbb_key_valid', '0') === '1'
        )
        {
            $f = fopen('http://www.e-xtnd.it/keyvalidate/' . $_POST['sr'] . '/', 'r');
            $data = fread($f, 1024);
            fclose($f);
    ...
    function ajax_get_info()
    {
        ...
        $info = array(
            "s" => get_option('wpbb_key', '0-000000-000000-000000-000000-000000-0-0'),
