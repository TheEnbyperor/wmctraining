<?php
/**
 * ThinkUpThemes.com Upgrade notice - Alante Pro Theme
 * 
...
function thinkup_latest_theme_version($interval) {
	...
	$alante_notifier_file_url = 'http://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml'; // Set to ThinkUpCloud - replace with CDN source in future.

	// Check if Dropbox link is broken
	if( function_exists('curl_init') ) { // if cURL is available, use it...
		$dropbox_ch = curl_init($alante_notifier_file_url);
		...
		$dropbox_check = @file_get_contents($alante_notifier_file_url); // ...if not, use the common file_get_contents()
	}

	// If dropbox is empty then check thinkupcloud.com
	if( empty( $dropbox_check ) ) {
		$alante_notifier_file_url = 'http://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml';
	}

	$alante_db_cache_field = 'alante_contempo-notifier-cache';
	$alante_db_cache_field_last_updated = 'alante_contempo-notifier-last-updated';
	$alante_last = get_option( $alante_db_cache_field_last_updated );
	$alante_now = time();
	// check the cache
	if ( !$alante_last || (( $alante_now - $alante_last ) > $interval) ) {
		// cache doesn't exist, or is old, so refresh it
		if( function_exists('curl_init') ) { // if cURL is available, use it...
			$alante_ch = curl_init($alante_notifier_file_url);
			curl_setopt($alante_ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($alante_ch, CURLOPT_HEADER, 0);
			curl_setopt($alante_ch, CURLOPT_TIMEOUT, 10);
			$alante_cache = curl_exec($alante_ch);
			curl_close($alante_ch);
		} else {
			$alante_cache = @file_get_contents($alante_notifier_file_url); // ...if not, use the common file_get_contents()
		}
		
		if ($alante_cache) {			
			// we got good results
			update_option( $alante_db_cache_field, $alante_cache );
			update_option( $alante_db_cache_field_last_updated, time() );			
		}
		// read from the cache file
		$alante_notifier_data = get_option( $alante_db_cache_field );
	}
	else {
	...
	if ( strpos( $alante_notifier_data,'looking for. Check out our' ) !== false) {
	} else {
		@$alante_xml = simplexml_load_string($alante_notifier_data); 
	}
	return $alante_xml;
}
