...
// Add shortcodes
	...
	function add_metaboxes( $slug, $post = false ){
		...
		if(!empty($post)){
			if(!in_array($post->post_type, array('post','page','client','portfolio','team','testimonial'))){return;}
		}else{
				$screen = get_current_screen();
				if(!in_array($screen->base, array('post','page','client','portfolio','team','testimonial'))){return;}
			}
...
<option value=\"list_font_adv\">".__('Advanced list (Font Awesome)','thinkupshortcodes')."</option>
 ...
 <select style=\"display:none;\" class=\"thinkupshortcodes-elements-selector\" id=\"thinkupshortcodes-elements-selector-pricingtable\">				<option value=\"\"></option><option value=\"pricing1\">".__('Fixed','thinkupshortcodes')."</option>
