...
if ( ! class_exists( 'thinkup_widget_popularposts' ) ) {
	class thinkup_widget_popularposts extends WP_Widget {
		...
		$comment_input = NULL;
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Popular Posts', 'alante' ) : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
			...
			while ($posts->have_posts()) : $posts->the_post();
				...
				if ( $comment_count == 0 ) {
					$commentnumber_input = __( 'No Comments', 'alante' );
				} else if ( $comment_count == 1 ) {
					$commentnumber_input = __( '1 Comment', 'alante' );
				} else if ( $comment_count > 1 ) {
					$commentnumber_input = $comment_count . ' ' . __( 'Comments', 'alante' );
				}
				if ( $instance['comments'] == 'on' ) {
					$comment_input = thinkup_input_commentspopuplink( __( 'No Comments', 'alante' ), __( '1 Comment', 'alante' ), __( '% Comments', 'alante' ), 'comment' );
				}			
