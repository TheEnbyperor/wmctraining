...
if ( ! class_exists( 'thinkup_widget_categories' ) ) {
	class thinkup_widget_categories extends WP_Widget {
		...
		function form($instance) {
			...
			echo '<p><label for="' . $this->get_field_id('exclude') . '">Exclude categories: <input class="widefat" id="' . $this->get_field_id('exclude') . '" name="' . $this->get_field_name('exclude') . '" type="text" value="' . attribute_escape($exclude) . '" style="width: 95px;margin-left: 20px;" /></label></p>';
		}
		...
		function widget($args, $instance) {
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Categories', 'alante' ) : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
					...
					echo '<ul>';
						foreach( $terms as $term ) {
						  echo '<li class="cat-item cat-item-24">',
								'<a href="' . get_category_link( $term->term_id ) . '" title="' . sprintf( __( 'View all posts in %s', 'alante' ), $term->name ) . '" ' . '>' . $term->name;  
									if ($instance['postswitch'] == 'on') {
										echo '<span><small>('.$term->count.')</small></span>';
									}
						  echo 	'</a></li> ';
						}
					echo '</ul>';
