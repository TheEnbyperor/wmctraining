...
abstract class Source
{
	...
	public static function invalidArray(&$array, $format, $strictFormat=true, $strictArray=true, $translations=array())
	{
		foreach( $array as $k => $v )
		{
			if( isset($format[$k]) )
			{
				$err = self::invalidValue($format[$k], $v, isset($translations[$k]) ? $translations[$k] : $k);
				if( $err !== false )
					return $err;
			}
			else if( $strictFormat )
			{
				return sprintf(_x("Unknown entry %s", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), isset($translations[$k]) ? $translations[$k] : $k);
			}
		}
		...
		foreach( $format as $k => $descr )
		{
			...
			if( !isset($array[$k]) && strpos($opt[0], 'o') === false )
			{
				if( strpos($opt[0], 'a') !== false )
				{
					...
					return sprintf(_x("Missing entry %s", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), isset($translations[$k]) ? $translations[$k] : $k);
			}
		}
	...
	private static function invalidValue($format, $v, $translation='[undefined]')
	{
		$f = substr($format, 0, 1);
		...
		{
			if( !is_array($v) )
				return sprintf(_x("%s is not an array", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
			else
			{
				$sub = explode('/', $f);
				if( strpos($sub[0], '+') !== false && empty($v) )
					return sprintf(_x("At least one %s must be set", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);

				if( count($sub) > 1 )
				{
					foreach( $v as $item )
						return self::invalidValue($sub[1], $item, $translation);
				}
			}
		}
		else if( $f == "/" )
		{
			if( !preg_match($format, $v) )
				return sprintf(_x("%s is not valid", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
		}
		else if( $f == 'i' )
		{
			if( strlen(trim($v))==0 )
			{
				if( strpos($opt, 'o') === false && strpos($opt, 'a') === false )
					return sprintf(_x("%s needs a value", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
			}
			else if( !is_numeric($v) )
				return sprintf(_x("%s is not a number", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
			else if( strpos($opt, '+') !== false && $v <= 0 )
				return sprintf(_x("%s must be greater than zero", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
			else if( strpos($opt, '0') !== false && $v < 0 )
				return sprintf(_x("%s must be equal or greater than zero", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
		}
		else
		{
			if( !is_string($v) )
				return sprintf(_x("%s is not a string", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
			else if( strpos($opt, '+') !== false && empty($v) )
				return sprintf(_x("%s is empty", "Input array validation", LWS_ADMIN_PANEL_DOMAIN), $translation);
		}
		return false;
	}
