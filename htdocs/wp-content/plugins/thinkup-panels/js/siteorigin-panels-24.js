...
module.exports = panels.view.dialog.extend( {
	...
	render: function () {
		...
		if ( ! _.isUndefined( panelsOptions.widgets[this.model.get( 'class' )] ) ) {
			...
			this.$('.so-title .widget-name').closest('div[class^="so-panels-dialog"]').find('.so-content').first().addClass( 'thinkup-content-' + panelsOptions.widgets[ this.model.get('class')].id ); // ThinkUpThemes customization - allows id to be added to so-content class to style ThinkUp Builder widgets. 
		} else {
	...
	getFormValues: function ( formSelector ) {
		...
		$f.find( '[name]' ).each( function () {
			...
			try {
				...
				// Added to ensure compatibility with Caldera widgets
				if( name === null){return;}
