			...
			<div class="style-section-fields" style="display: none">
				...
				foreach( $fields as $field_id => $field ) {
					...
					if($field['group'] == $group_id){
						?>
						<div class="style-field-wrapper <?php echo 'thinkup-style-field-' . $field['type'] . ' ' . $field_id; ?>">
	...
	switch($field['type']) {
		...
		case 'image' :
			...
			<div class="so-image-selector">
				<div class="current-image" <?php if( !empty($image) ) echo 'style="background-image: url(' . $image . ');"'; ?>>
				</div>

				<div class="select-image">
					<?php _e('Select Image', 'siteorigin-panels') ?>
				</div>
				<input type="hidden" name="<?php echo esc_attr($field_name) ?>" value="<?php echo $current ?>" />
			</div>
